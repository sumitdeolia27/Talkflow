...
export const getUserData = async (req, res) => {
    try {
        ...
        if (!user) {
            ...
            let full_name = req.authUser.name || 'User'
            let profile_picture = req.authUser.picture || ''

            if (!req.authUser.name) {
                const firebaseAdmin = initFirebase();
                const firebaseUser = await firebaseAdmin.auth().getUser(userId)
                full_name = firebaseUser.displayName || full_name
                profile_picture = firebaseUser.photoURL || profile_picture
