...
var WebSocketWorkerConnection = class {
	...
	options;
	strategy;
	debugLog = (0, debug.default)("inngest:connect");
	constructor(options) {
	...
	applyDefaults(opts) {
		...
		const env = require_env.getProcessEnv();
	...
	*/
	async connect(attempt = 0) {
		...
		const hashedSigningKey = this.options.signingKey ? require_strings.hashSigningKey(this.options.signingKey) : void 0;
		...
		const hashedFallbackKey = this.options.signingKeyFallback ? require_strings.hashSigningKey(this.options.signingKeyFallback) : void 0;
		const capabilities = {
			trust_probe: "v1",
			connect: "v1"
		};
