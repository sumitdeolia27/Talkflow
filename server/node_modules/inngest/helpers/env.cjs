...
const devServerHost = (env = getProcessEnv()) => {
	return [
		require_consts.envKeys.ReactAppInngestBaseUrl,
		require_consts.envKeys.NextPublicInngestBaseUrl,
		require_consts.envKeys.ReactAppInngestDevMode,
		require_consts.envKeys.NextPublicInngestDevMode
	].map((key) => {
		return env[key];
	}).find((v) => {
		if (!v) return;
...
const prodChecks = [
	[
		require_consts.envKeys.IsCloudflarePages,
		"equals",
		"1"
	],
	[
		require_consts.envKeys.Context,
		"starts with",
		"prod"
	],
	[
		require_consts.envKeys.Environment,
		"starts with",
		"prod"
	],
	[
		require_consts.envKeys.NodeEnv,
		"starts with",
		"prod"
	],
	[
	...
	[require_consts.envKeys.DenoDeployment, "is truthy"],
	[
		require_consts.envKeys.VercelEnvKey,
		"is truthy but not",
		"development"
...
const processEnv = (key) => {
	if (!Object.values(require_consts.envKeys).includes(key)) throw new Error(`Env var "${key}" is not in the whitelist`);
